...
def show_plate(img, cluster_num, color_list):
    square_len = 50

    def form_square(hsv_color):
        return np.tile(hsv_color[np.newaxis, np.newaxis, :], (square_len, square_len, 1))
    ...
    for ci in range(cluster_num):
        print(color_list[ci, 0], color_list[ci, 1], color_list[ci, 2], sep=', ')

    cv.imwrite('dominent_colors2.jpg', color_plate)


def get_dominent_colors(img, cluster_num=5, cluster='kmeans', if_show=False):
    ...
    for c in range(cluster_num):
        color_list[c] = centers[c].astype('int')

    if if_show:
        show_plate(img, cluster_num, color_list)
